...
# Daisy Pandey, September 8, 2020, Modified to add Track class, added methods to CD class to handle tracks
# Daisy Pandey, September 9, 2020, Added structured error handling to handle errors
#------------------------------------------#
...
class CD:
    ...
    def rmv_track(self, track_id: int) -> None:
        ...
        try:
            ...
            print('====Error!!=====')
            print(f'You entered {track_id}, which is not a valid entry for track ID.')
            print('Please enter a correct track ID from above list.')
            print()
            return    
            
        intRowNr = -1
        blnCDRemoved = False
        for row in self.__tracks:
            intRowNr += 1
            if row == None:
                continue
            if trkId == row.position:           
                del self.__tracks[intRowNr]
                blnCDRemoved = True
                break
        if blnCDRemoved:
            ...
            print('Could not find this track!')
        return
